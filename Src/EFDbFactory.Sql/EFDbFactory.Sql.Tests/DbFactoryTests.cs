...
namespace EFDbFactory.Sql.Tests
{
    ...
    {
        private const string _connString = "Server=localhost\\sqlexpress;Database=QuizDb;Integrated Security=True;Trusted_Connection=True;MultipleActiveResultSets=True;ConnectRetryCount=0";

        private static IDbFactory GetNoCommitFactory() => new DbFactory(_connString).CreateNoCommit().GetAwaiter().GetResult();
